...
ดึงค่า `Software-Version` จาก [`VERSION.md`](https://github.com/bemindlabs/BWOC-Framework/blob/main/VERSION.md) แล้วรวมไว้ในข้อยืนยันเกี่ยวกับ CLI flags, manifest fields หรือพฤติกรรม ณ เวลาที่ index handbook ครั้งล่าสุด software version คือ **2.24.0** แต่ตัวเลขนี้เพิ่มขึ้นอัตโนมัติทุกครั้งที่แก้โค้ด — ถือว่าเป็นค่าเก่าและอ่าน `VERSION.md` ใหม่เมื่อคำตอบขึ้นกับเวอร์ชัน
...
- **Cargo SemVer** (`MAJOR.MINOR.PATCH` เช่น `2.24.0`) — internal development checkpoint, auto-bump ทุก edit แสดงผลโดย `bwoc --version` และ `VERSION.md Software-Version`
- **Release CalVer** (`vYYYY.M.D-<patch>` เช่น `v2026.6.6-0`) — public release identity, กำหนดโดย Git tag แสดงใน GitHub Releases
...
**Versioning:**
- Software version ณ เวลาที่เขียน handbook นี้: **2.24.0** (Cargo SemVer) ถือว่าเก่าแล้ว อ่าน `VERSION.md` ใหม่
- Public release ล่าสุด ณ เวลาที่เขียน handbook นี้: `v2026.6.6-0` (CalVer) ถือว่าเก่าแล้ว ตรวจ GitHub Releases
...
- อย่าสับสน Cargo SemVer (`2.24.0`) กับ CalVer release tag (`v2026.6.6-0`) สอง namespace นี้เป็นอิสระและมี semantic ต่างกัน
